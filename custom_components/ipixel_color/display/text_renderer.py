...
def render_text_to_png(text: str, width: int, height: int, antialias: bool = True, font_size: float | None = None, font: str | None = None, line_spacing: int = 0, text_color: str = "ffffff", bg_color: str = "000000") -> bytes:
    ...
    if text[0] == '#':
        img = Image.new("RGB", (width, height), init_zero)
    else:
        if not antialias:
    ...
    if text[0] == '#':
        ...
        def right_x(text: str) -> int:
            left, _, right, _ = draw.textbbox((0, 0), text, font=font)
            w = right - left
            return 32 - w

        draw.text((right_x(s1), y1), s1, font=font, fill=c1)
        draw.text((right_x(s2), y2), s2, font=font, fill=c2)
        draw.text((right_x(s3), y3), s3, font=font, fill=c3)
        draw.text((right_x(s4), y4), s4, font=font, fill=c4)
